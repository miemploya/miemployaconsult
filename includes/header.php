...
<body class="font-sans bg-slate-50 text-slate-900 antialiased" x-data="{ mobileMenu: false }">
...
<nav class="nav-glass fixed top-0 left-0 right-0 z-50 transition-all duration-300" id="mainNav">
        ...
        <div class="flex items-center justify-between h-16 lg:h-20">
            ...
            <a href="<?= SITE_URL ?>/index.php" class="flex items-center gap-3 group shrink-0">
                ...
                <div class="hidden sm:block">
                    <p class="text-lg font-black text-slate-900 tracking-tight leading-tight"><?= SITE_NAME ?></p>
                    <p class="text-[10px] text-slate-400 font-medium uppercase tracking-widest"><?= COMPANY_TAGLINE ?></p>
                </div>
            </a>
            ...
            <div class="hidden lg:flex items-center gap-1">
                ...
                <a href="<?= SITE_URL ?>/<?= $item[1] ?>.php" 
                   class="px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 <?= $is_active ? 'text-brand-600 bg-brand-50' : 'text-slate-600 hover:text-brand-600 hover:bg-slate-50' ?>">
                    <?= $item[0] ?>
                </a>
                <?php endforeach; ?>
                ...
                <div class="relative" x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false">
                    <button @click="open = !open"
                            class="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 
                                   <?= in_array($current_page, $dropdown_pages) ? 'text-brand-600 bg-brand-50' : 'text-slate-600 hover:text-brand-600 hover:bg-slate-50' ?>">
                        Resources
                    ...
                    <div x-show="open"
                         ...
                         x-cloak
                         class="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-64 bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-slate-100 overflow-hidden z-50">
                        <div class="p-2">
                            ...
                            <a href="<?= SITE_URL ?>/<?= $dd[1] ?>.php"
                               class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm transition-all duration-200 group
                                      <?= $is_dd_active ? 'text-brand-600 bg-brand-50' : 'text-slate-600 hover:text-brand-600 hover:bg-slate-50' ?>">
                                <span class="w-9 h-9 rounded-lg bg-gradient-to-br from-brand-50 to-purple-50 border border-slate-100 flex items-center justify-center shrink-0 group-hover:border-brand-200 transition-colors">
                                    <i data-lucide="<?= $dd[2] ?>" class="w-4 h-4 text-brand-500"></i>
                                </span>
                                <div>
                                    <p class="font-semibold leading-tight"><?= $dd[0] ?></p>
                                    <p class="text-[11px] text-slate-400 mt-0.5"><?= $dd[3] ?></p>
                                </div>
                            </a>
                            <?php endforeach; ?>
                        </div>
                ...
                <a href="<?= SITE_URL ?>/contact.php" 
                   class="px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 <?= $current_page === 'contact' ? 'text-brand-600 bg-brand-50' : 'text-slate-600 hover:text-brand-600 hover:bg-slate-50' ?>">
                    Contact
                </a>
            </div>
            ...
            <div class="hidden lg:flex items-center gap-3">
                <?php if (isLoggedIn()): ?>
                    <?php if (isAdmin()): ?>
                    <a href="<?= SITE_URL ?>/admin/index.php" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-brand-600 bg-brand-50 hover:bg-brand-100 rounded-xl transition-all">
                        <i data-lucide="layout-dashboard" class="w-4 h-4"></i> Dashboard
                    </a>
                    <?php endif; ?>
                    <a href="<?= SITE_URL ?>/auth/logout.php" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-slate-600 hover:text-red-600 rounded-xl transition-all">
                        <i data-lucide="log-out" class="w-4 h-4"></i> Logout
                    </a>
                <?php else: ?>
                    <a href="<?= SITE_URL ?>/auth/login.php" class="px-4 py-2 text-sm font-semibold text-slate-600 hover:text-brand-600 transition-colors">Sign In</a>
            ...
            <!-- Mobile Menu Toggle -->
            <button @click="mobileMenu = !mobileMenu" class="lg:hidden p-2 rounded-xl hover:bg-slate-100 transition-colors">
                <i data-lucide="menu" class="w-6 h-6 text-slate-700" x-show="!mobileMenu"></i>
                <i data-lucide="x" class="w-6 h-6 text-slate-700" x-show="mobileMenu" x-cloak></i>
            </button>
        </div>
    </div>
    ...
    <div x-show="mobileMenu" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-2" x-cloak
         class="lg:hidden bg-white/95 backdrop-blur-xl border-t border-slate-100 shadow-xl">
        <div class="max-w-7xl mx-auto px-4 py-4 space-y-1">
            ...
            ?>
            <a href="<?= SITE_URL ?>/<?= $item[1] ?>.php" 
               class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all <?= $is_active ? 'text-brand-600 bg-brand-50' : 'text-slate-600 hover:bg-slate-50' ?>">
